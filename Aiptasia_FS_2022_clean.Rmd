...
```{r}
setwd("/Users/mariaingersoll/Desktop/BU_Research/Davies-Gilmore/Aip_FS_2022/Aiptasia_Fed_Starved")
```
...
```{r}
countData <- read.table("AiptasiaFS_2022_counts_2.txt")
length(countData[,1])
#26979
head(countData)
...
#save as rawcounts_220123 as 6x4 landscape
...
```{r}
countData_s <- read.table("Symbiont_FS_2022_counts.txt")
head(countData_s)
...
#save as rawcounts_symbionts_080223 as 7x4 landscape
...
v = setwd("/Users/mariaingersoll/Desktop/BU_Research/Davies-Gilmore/Aip_FS_2022")
...
summary(res_SS_SF)
# out of 26413 with nonzero total read count
# adjusted p-value < 0.1
# LFC > 0 (up)       : 951, 3.5%
# LFC < 0 (down)     : 1327, 4.9%
...
#saved as plotma_apo as landscape 5x4
plotMA(res_SS_SF, main="SS vs SF")
#saved as plotma_sym as landscape 5x4
```

Now make a table of the differential expression values (log2fc) between AF, AS and SF, SS
```{r}
write.table(res_AS_AF, file="AS_AF_DE_22.txt", quote=F, sep="\t")
head(read.delim("AS_AF_DE_22.txt"))
res_AS_AF<- read.delim("AS_AF_DE_22.txt")
head(res_AS_AF)
...
write.table(res_SS_SF, file="SS_SF_DE_22.txt", quote=F, sep="\t")
head(read.delim("SS_SF_DE_22.txt"))
res_SS_SF<-read.delim("SS_SF_DE_22.txt") 
head(res_SS_SF)
...
#saved as correlation_as.pdf as landscape 5x4
```
...
#FALSE  TRUE 
# 8893 18086 
write.csv(rldpvals_apo, "mingers_AS_AF_RLDandPVALS_22.csv", quote=F)

rldpvals_sym=cbind(rld_sym,val_SS_SF)
head(rldpvals_sym)
dim(rldpvals_sym)
table(complete.cases(rldpvals_sym))
write.csv(rldpvals_sym, "mingers_SS_SF_RLDandPVALS_22.csv", quote=F)
```

Read back in the csv files you just made to prep for PCA
```{r}
rldpvals_apo <- read.csv(file="mingers_AS_AF_RLDandPVALS_22.csv", row.names=1)
head(rldpvals_apo)
rld_apo <- rldpvals_apo[,1:10]
head(rld_apo)

rldpvals_sym <- read.csv(file="mingers_SS_SF_RLDandPVALS_22.csv", row.names=1)
head(rldpvals_sym)
...
# saved as landscape with 6x4 as pca_012223.pdf
...
plast_plot <- ggplot(data = plast_out_all, aes(x = treat, y = dist)) + 
  ...
  theme(panel.background = element_rect(color="black", fill="white"),
        ...
        legend.key.size=unit(10,"point"))
plast_plot
#saved as plasticity_012223 as landscape with 6x4
...
```{r}
res_apo = read.table("AS_AF_DE_22.txt")
head(res_apo)
res_apo = res_apo %>%
...
res_sym = read.table("SS_SF_DE_22.txt")
head(res_sym)
res_sym = res_sym %>%
  tibble::rownames_to_column(var = "gene_ID")
head(res_sym)
...
head(go_input_apo)
write.csv(go_input_apo, file="apo_GO_22.csv", quote=F, row.names=FALSE)
...
head(go_input_sym)
write.csv(go_input_sym, file="sym_GO_22.csv", quote=F, row.names=FALSE)
```
...
#saved as CCapo_012223.pdf as portrait with 30x10 dimensions
resultsCC
...
#saved as BPapo_012223 as portrait with 100x10 dimensions
...
#saved as MFapo_012223.pdf as portrait with 45x10 dimenstions
resultsMF
```
...
#saved as CCsym_012223.pdf as portrait with 30x10 dimensions
resultsCC
...
#saved as BPsym_012223.pdf as portrait with 75x10 dimensions
resultsBP
...
#saved as MFsym_012223 as portrait with dimensions 25x10
resultsMF
```
...
```{r}
iso2go = read.table("aiptasia_iso2go.tab", sep="\t")
colnames(iso2go) <- c("gene_ID", "GO.terms")
head(iso2go)

newnames = c("gene_ID", "accession", "description" )
iso2gene = read.delim("aiptasia_iso2gene.txt", sep="\t", header=TRUE)
...
```{r}
apo_DE = read.csv("mingers_AS_AF_RLDandPVALS_22.csv")
head(apo_DE)
apo_DE= apo_DE %>%
...
sym_DE = read.csv("mingers_SS_SF_RLDandPVALS_22.csv")
head(sym_DE)
sym_DE= sym_DE %>%
  mutate(gene_ID = gsub("AIPGENE*", "isogroup", X)) %>%
  dplyr::select(-X) %>%
...
-Apo first
```{r}
bp_apo = read.csv("BP_apo_GO_22.csv", sep="\t")
head(bp_apo)
...
pheatmap(as.matrix(explc_nfkb_sym),
         ...
         scale="none",
         legend=F)
#saved as pheatmap_nfkb_sym_012223.pdf as portrait with 6x4 dimensions
```
...
pheatmap(as.matrix(explc_rosd_apo),
         ...
         scale="none",
         legend=T)
#saved as pheatmap_ros_apo_012223.pdf as portrait with 6x4 dimensions
```
...
pheatmap(as.matrix(explc_rosd_sym),
         ...
         scale="none",
         legend=F)
#saved as pheatmap_ros_sym_012223.pdf as portrait with 5x4 dimensions
```
...
#saved as venn_012323.pdf as landscape with 7x7 dimensions
```
...
```{r}
aip_kog_2 <- read.delim("aip_kog_mvi_final.txt", header=TRUE)
head(aip_kog_2)
nrow(aip_kog_2)
#15345
...
oc_kog<-read.table("oculina_uniq_kog.tab.txt", header=TRUE)
head(oc_kog)
nrow(oc_kog)
#5007
...
nv_kog <- read.delim("nv_kog_mvi_final.txt", header=TRUE)
head(nv_kog)
nrow(nv_kog)
# 14606
...
```{r}
ocu_sgp<-read.csv("ocu_ufct_signedP.csv", header=TRUE)
head(ocu_sgp)

aips_sgp <- read.csv("sym_GO_22.csv", header=TRUE)
head(aips_sgp)
z <- c("genes","signedlogP")
colnames(aips_sgp)=paste(z)
head(aips_sgp)

aipa_sgp <- read.csv("apo_GO_22.csv", header=TRUE)
head(aipa_sgp)
z <- c("genes","signedlogP")
colnames(aipa_sgp)=paste(z)
head(aipa_sgp)

nv_sgp <- read.csv("Nv_SF_signedP.csv", header=TRUE)
head(nv_sgp)
...
# save this one for the manuscript, landscape 7 x 5 as KOG_012323
...
```{r}
aip_apo_goBP=read.table("MWU_BP_apo_GO_22.csv",header=T, sep = " ")
aip_sym_goBP=read.table("MWU_BP_sym_GO_22.csv",header=T, sep = " ")
oc_goBP=read.table("MWU_BP_heats_ufct_signedP.csv",header=T, sep = " ")
nv_goBP=read.table("MWU_BP_Nv_starved_GO.csv",header=T, sep = " ")
```
...
head(sigs_as_aa)
write.table(sigs_as_aa, file="sigs_as_aa_GO_name.txt", quote=F, sep="\t", row.names=FALSE)
sigs_as_aa <- read.table("sigs_as_aa_GO_name.txt", header=T, sep="\t")
head(sigs_as_aa)
...
# 4
sigs_as_oc
write.table(sigs_as_oc, file="sigs_as_oc_GO_name.txt", quote=F, sep="\t", row.names=FALSE)
sigs_as_oc <- read.table("sigs_as_oc_GO_name.txt", header=T, sep="\t")
head(sigs_as_oc)
...
# 5
sigs_aa_oc
write.table(sigs_aa_oc, file="sigs_aa_oc_GO_name.txt", quote=F, sep="\t", row.names=FALSE)
sigs_aa_oc <- read.table("sigs_aa_oc_GO_name.txt", header=T, sep="\t")
head(sigs_aa_oc)
...
#60
sigs_as_nv
write.table(sigs_as_nv, file="sigs_as_nv_GO_name.txt", quote=F, sep="\t", row.names=FALSE)
sigs_as_nv <- read.table("sigs_as_nv_GO_name.txt", header=T, sep="\t")
head(sigs_as_nv)
...
# 89
sigs_aa_nv
write.table(sigs_aa_nv, file="sigs_aa_nv_GO_name.txt", quote=F, sep="\t", row.names=FALSE)
sigs_aa_nv <- read.table("sigs_aa_nv_GO_name.txt", header=T, sep="\t")
head(sigs_aa_nv)
...
# 2
sigs_nv_oc
write.table(sigs_nv_oc, file="sigs_nv_oc_GO_name.txt", quote=F, sep="\t", row.names=FALSE)
sigs_nv_oc <- read.table("sigs_nv_oc_GO_name.txt", header=T, sep="\t")
head(sigs_nv_oc)
```
...
nrow(nv_apo_sym_oc)
#102
write.table(nv_apo_sym_oc, file="GOdeltarank_all.txt", quote=F, sep="\t", row.names=FALSE)
...
bubbleplot
#saved as bubbleplot_all_GOdeltarank as portrait with 17x10 dimensions
```
...
```{r}
starvation <- read.table("GOdeltarank_starvation.txt", header=T, sep="\t")
...
bubbleplot_s
#saved as bubbleplot_starv_GOdeltarank.pdf as landscape 9x7
```
...
```{r}
defense <- read.table("GOdeltarank_immune.txt", header=T, sep="\t")
...
bubbleplot_d <- ggplot(melted_defense, aes(x=variable, y=GO_Group, fill=factor(sign(value)), size=abs(value))) +
      ...
      theme(panel.grid.major.y = element_line(color=c("white", "red", "red", "black", "red", "red2"), linetype = c("solid", "dashed", "dashed", "dashed", "dashed", "dashed")),
        ...
        axis.text.y = element_text(color="black", size=10, face=c("plain", "bold", "bold", "bold","bold", "bold")),
        axis.title.x = ggtext::element_markdown(face="bold", size=12),
        axis.title.y = ggtext::element_markdown(face="bold",size=12))

bubbleplot_d
#saved as bubbleplot_def_GOdeltarank as landscape 9x3
```
...
#795
write.csv(aip_apo_goBP_sig, "aip_apo_goBP_sig.csv", quote=F)

head(aip_sym_goBP)
aip_sym_goBP_sig <- aip_sym_goBP[aip_sym_goBP$p.adj<=0.1,]
nrow(aip_sym_goBP_sig)
#483
write.csv(aip_sym_goBP_sig, "aip_sym_goBP_sig.csv", quote=F)
```
...
H2O2
```{r}
total_h2o2_input <- read.table("r1r2_h2o2_survfit_input.txt", header=T)
...
total_h2o2 <- ggsurvplot(total_sfit_h2o2, data=total_h2o2_survfit_input, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoFed","ApoStarved","SymFed","SymStarved"),
        ...
        legend.key.size=unit(10,"point"))
total_h2o2
#saved as total_h2o2_survplot_050923.pdf as landscape with 6x4 dim

#separate out each comparison
#SF to SS
...
h2o2_sfss <- ggsurvplot(total_sfit_h2o2_sfss, data=total_h2o2_survfit_input_sfss, alpha=0.6, size=1, censor.size=0, legend.labs=c("SymFed","SymStarved"),
        ...
        legend.text.align = 0),
        legend.key.size=unit(10,"point"))
#saved as sfss_h2o2_survplot_050923.pdf as landscape with 5x3 dim
...
h2o2_sfaf <- ggsurvplot(total_sfit_h2o2_sfaf, data=total_h2o2_survfit_input_sfaf, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoFed","SymFed"),
        ...
        legend.text.align = 0),
        legend.key.size=unit(10,"point"))
#saved as sfaf_h2o2_survplot_050923.pdf as landscape with 5x3 dim
...
h2o2_afas <- ggsurvplot(total_sfit_h2o2_afas, data=total_h2o2_survfit_input_afas, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoFed","ApoStarved"),
        ...
        legend.text.align = 0),
        legend.key.size=unit(10,"point"))
#saved as afas_h2o2_survplot_050923.pdf as landscape with 5x3 dim
...
h2o2_asss <- ggsurvplot(total_sfit_h2o2_asss, data=total_h2o2_survfit_input_asss, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoStarved","SymStarved"),
        ...
        legend.text.align = 0),
        legend.key.size=unit(10,"point"))
#saved as asss_h2o2_survplot_050923.pdf as landscape with 5x3 dim
...
Pa14
```{r}
total_pa14_input <- read.table("r1r2_pa14_survfit_input.txt", header=T)
...
total_pa14
#saved as total_pa14_survplot_050923.pdf as landscape with 6x4 dim
...
pa14_sfss <- ggsurvplot(total_sfit_pa14_sfss, data=total_pa14_survfit_input_sfss, alpha=0.6, size=1, censor.size=0, legend.labs=c("SymFed","SymStarved"),
        ...
        legend.text.align = 0),
        legend.key.size=unit(10,"point"))
#saved as sfss_pa14_survplot_050923.pdf as landscape with 5x3 dim
...
pa14_sfaf <- ggsurvplot(total_sfit_pa14_sfaf, data=total_pa14_survfit_input_sfaf, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoFed","SymFed"),
        ...
        legend.text.align = 0),
        legend.key.size=unit(10,"point"))
#saved as sfaf_pa14_survplot_050923.pdf as landscape with 5x3 dim
...
pa14_afas <- ggsurvplot(total_sfit_pa14_afas, data=total_pa14_survfit_input_afas, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoFed","ApoStarved"),
        ...
        legend.text.align = 0),
        legend.key.size=unit(10,"point"))
#saved as afas_pa14_survplot_050923.pdf as landscape with 5x3 dim
...
pa14_asss <- ggsurvplot(total_sfit_pa14_asss, data=total_pa14_survfit_input_asss, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoStarved","SymStarved"),
        ...
        legend.text.align = 0),
        legend.key.size=unit(10,"point"))
#saved as asss_pa14_survplot_050923.pdf as landscape with 5x3 dim
...
Smarc
```{r}
total_smarc_input <- read.table("r1r2_smarc_survfit_input.txt", header=T)
...
total_smarc
#saved as total_smarc_survplot_050923.pdf as landscape with 6x4 dim
...
smarc_sfss <- ggsurvplot(total_sfit_smarc_sfss, data=total_smarc_survfit_input_sfss, alpha=0.6, size=1, censor.size=0, legend.labs=c("SymFed","SymStarved"),
        ...
        legend.text.align = 0),
        legend.key.size=unit(10,"point"))
#saved as sfss_smarc_survplot_050923.pdf as landscape with 5x3 dim
...
smarc_sfaf <- ggsurvplot(total_sfit_smarc_sfaf, data=total_smarc_survfit_input_sfaf, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoFed","SymFed"), xlim=c(0,30), break.x.by=10,
        ...
        legend.text.align = 0),
        legend.key.size=unit(10,"point"))
#saved as sfaf_smarc_survplot_050923.pdf as landscape with 5x3 dim
...
smarc_afas <- ggsurvplot(total_sfit_smarc_afas, data=total_smarc_survfit_input_afas, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoFed","ApoStarved"),
        ...
        legend.text.align = 0),
        legend.key.size=unit(10,"point"))
#saved as afas_smarc_survplot_050923.pdf as landscape with 5x3 dim
...
smarc_asss <- ggsurvplot(total_sfit_smarc_asss, data=total_smarc_survfit_input_asss, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoStarved","SymStarved"),
        ...
        legend.text.align = 0),
        legend.key.size=unit(10,"point"))
#saved as asss_smarc_survplot_050923.pdf as landscape with 5x3 dim
