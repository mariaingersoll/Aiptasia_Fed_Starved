...
```{r}
countData <- read.table("./Data_Files/AiptasiaFS_2022_counts.txt")
length(countData[,1])
#26974
head(countData)
...
#save as rawcounts_symbionts_071823 as 6.5x4 landscape in Figure directory
...
# FALSE=22385 TRUE=4588

table(res_SS_SF$padj<0.1)
# FALSE=10056 TRUE=2274
...
# FALSE=8892 TRUE=18082

table(complete.cases(val_SS_SF))
# FALSE=14644 TRUE=12330
```
...
pca_s$Clone = c("A1", "A1", "A2", "A2", "A3", "A3", "A4", "A4", "A5", "A5", "S1", "S1", "S2", "S2", "S3", "S3", "S4", "S4", "S5", "S5")
...
pca_plot <- ggplot(pca_s, aes(PC1, PC2, color = Nutrition_Status, fill=treat, pch = Clone, group=treat)) +
  geom_point(size=2) +
...
- do starved vs apo fed and sym starved vs sym fed with the fed as the control
```{r}
...
plast_out_all <- PCAplast(pca = pca_df_plast, # the PCA dataframe containing the PCA eigenvalues
        ...
        num_pca =  2, # the number of PCAs to include in analysis (default is 'all', but you can specify another number with a minimum of 2 PCAs)
         ...
         group = "Clone", # the grouping column (i.e., colony). If blank, will assume control level grouping only! was "symstat" in first submission; changed to Clone here
...
plast_plot <- ggplot(data = plast_out_all, aes(x = treat, y = dist, pch=Clone, group=treat)) + 
  geom_point(color="purple", stroke=1, position = position_jitter(width = 0.1),aes(fill=symstat)) +
 stat_summary(fun.data = "mean_sdl", fun.args = list(mult = 1), geom = "errorbar", width = 0, colour = c("black")) +
  ...
  scale_shape_manual(values=sym_shape, guide=guide_legend(override.aes = list(fill=list("white","white","white","white","white","purple","purple","purple","purple","purple")))) +
  ylim(0,60)+
  ylab("Plasticity")+
  xlab("Treatment")+
  ...
  theme(panel.background = element_rect(color="black", fill="white"),
        ...
        legend.key.size=unit(10,"point"))
plast_plot
#saved as plasticity_012223 as portrait with 4x3 in Figures
...
#symstat      1 1092.7  1092.7   38.24 0.000264 ***
#Residuals    8  228.6    28.6                     
#---
#Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

####test assumptions for aov

#homogeneity of variances
plot(plast_aov, 1)
#three outliers
#so check their significance as outliers
leveneTest(dist ~ symstat, data = plast_out_all)
#      Df F value Pr(>F)
#group  1  0.8978 0.3711
#       8                 
#From leveneTest we can see that the p-value is not less than the significance level of 0.05. This means that there is no evidence to suggest that the variance across groups is statistically significantly different. Therefore, we can assume the homogeneity of variances in the different treatment groups.

#check normality
plot(plast_aov, 2)
# Extract the residuals
aov_residuals <- residuals(object = plast_aov)
# Run Shapiro-Wilk test
shapiro.test(x = aov_residuals )
#Shapiro-Wilk normality test
#data:  aov_residuals
#W = 0.90677, p-value = 0.2595
...
```{r}
go_bp_apo <- read.csv("./Data_Files/MWU_BP_apo_GO_22.csv", header=TRUE, sep=" ")
head(go_bp_apo)
write.table(go_bp_apo, "./Data_Files/MWU_BP_apo_GO_22.txt", quote=F, sep="\t", row.names=FALSE)

go_bp_sym <- read.csv("./Data_Files/MWU_BP_sym_GO_22.csv", header=TRUE, sep=" ")
head(go_bp_sym)
write.table(go_bp_sym, "./Data_Files/MWU_BP_sym_GO_22.txt", quote=F, sep="\t", row.names=FALSE)
```
...
prettyvenn05=venn.diagram(
  x = candidates_all05,
  category.names=c("Apo (4860)", "Sym (1716)"),
  col=c("purple3","purple3"),
  lwd=c(3,3),
  filename=NULL,
  #main = "DEGs Under Starvation",
...
Looking at the shared 1460 degs, seeing if they go in the same direction in apo and sym
```{r}
...
Now doing GO analysis on the Apo only, Sym only, and shared DEGs (via Fisher Exact test, following Dan Wuitchik's code)

- First format your input files
```{r}
#starting with just the shared ones because those were easiest to pull out from above; doesn't matter whether we do apo or sym for this because they have the same
head(shared_apo)
shared_fisher <- shared_apo
shared_fisher <- rownames_to_column(shared_fisher, var="gene_ID")
head(shared_fisher) 
shared_fisher <- shared_fisher %>%
  mutate(gene_ID = gsub("AIPGENE","isogroup", gene_ID)) %>%
  mutate(Fishers = 1) %>%
...
#now trying to isolate the just apo and just sym ones
apo_DEGs05_DF <- as.data.frame(apo_DEGs05) %>%
  mutate(gene_ID = gsub("AIPGENE","isogroup", apo_DEGs05)) %>%
  mutate(Fishers_apo = 1)%>%
  dplyr::select(gene_ID, Fishers_apo)
sym_DEGs05_DF <- as.data.frame(sym_DEGs05) %>%
  mutate(gene_ID = gsub("AIPGENE","isogroup", sym_DEGs05)) %>%
  mutate(Fishers_sym = 1)%>%
  dplyr::select(gene_ID, Fishers_sym)
fj <- full_join(apo_DEGs05_DF, sym_DEGs05_DF) %>%
  full_join(iso2go)
tail(fj)

fishers_apo_only <- fj %>%
  mutate(Fishers_apo = if_else(is.na(Fishers_apo), 0, 1)) %>%
  select(gene_ID, Fishers_apo)
write.csv(fishers_apo_only, file="./Data_Files/fishers_apo_only_GO.csv", quote=F, row.names=FALSE)

fishers_sym_only <- fj %>%
  mutate(Fishers_sym = if_else(is.na(Fishers_sym), 0, 1)) %>%
  select(gene_ID, Fishers_sym)
write.csv(fishers_sym_only, file="./Data_Files/fishers_sym_only_GO.csv", quote=F, row.names=FALSE)
```
Then do GO enrichment analysis on the Fisher files in the script "Fisher_MWU.R"

Reformatting the output files to be more easily read
```{r}
output_1 <- read.csv("./Data_Files/MWU_BP_shared_fisher_GO.csv", header=TRUE, sep=" ")
head(output_1)
write.table(output_1, "./Data_Files/MWU_BP_shared_fisher_GO.txt", quote=F, sep="\t", row.names=FALSE)

output_2 <- read.csv("./Data_Files/MWU_BP_fishers_apo_only_GO.csv", header=TRUE, sep=" ")
head(output_2)
write.table(output_2, "./Data_Files/MWU_BP_fishers_apo_only_GO.txt", quote=F, sep="\t", row.names=FALSE)

output_3 <- read.csv("./Data_Files/MWU_BP_fishers_sym_only_GO.csv", header=TRUE, sep=" ")
head(output_3)
write.table(output_3, "./Data_Files/MWU_BP_fishers_sym_only_GO.txt", quote=F, sep="\t", row.names=FALSE)
```
...
dim(sigs_as_aa)
# 397 GO terms that are significant and shared between both by name
head(sigs_as_aa)
...
head(starvation)
melted_starv <- melt(starvation, id.vars = "GO_Group")
head(melted_starv)
nrow(melted_starv)
#164
...
#defense
melted_defense <- melt(defense, id.vars = "GO_Group")
head(melted_defense)
nrow(melted_defense)
#24
...
```
Trying to make those two graphs into one fig
```{r}
figure <- ggarrange(bubbleplot_s, bubbleplot_d,
                    ncol = 1, nrow = 2, common.legend = TRUE, legend="right", heights=c(3,1))

#saved as bubbleplot_starv_deg_072323 as portrait with 9x6.5
```
...
#Making survival curves
- Input files have 1=alive and 2=dead in the Mortality column; Day column has the day at which death occurred or 28 for surviving the whole thing
...
total_h2o2 <- ggsurvplot(total_sfit_h2o2, data=total_h2o2_survfit_input, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoFed","ApoStarved","SymFed","SymStarved"),
        ...
        legend.key.size=unit(10,"point"))
total_h2o2
#saved as total_h2o2_survplot_050923.pdf as landscape with 5x3 dim in Figures
...
total_cox_h2o2_1 <- coxph(Surv(time = total_h2o2_input$Day, event = total_h2o2_input$Mortality) ~ Treat+frailty(factor(total_h2o2_input$trial)), cluster=Treat, data=total_h2o2_input)
summary(total_cox_h2o2_1)
#TO AF
#                           coef  se(coef) se2    Chisq   DF p       
#TreatAS                   3.971 0.202412 0.7272   384.9 1   1.1e-85
#TreatSF                   1.532 0.009887 0.7818 24002.5 1   0.0e+00
#TreatSS                   4.041 0.156465 0.7266   667.0 1  4.5e-147
#frailty(factor(total_h2o2                           0.3 0   3.3e-01
#Likelihood ratio test= 147  on 3 df,   p=<2e-16

total_h2o2_input$Treat <- relevel(total_h2o2_input$Treat, "AS")
total_cox_h2o2_2 <- coxph(Surv(time = total_h2o2_input$Day, event = total_h2o2_input$Mortality) ~ Treat+frailty(factor(total_h2o2_input$trial)), cluster=Treat, data=total_h2o2_input)
summary(total_cox_h2o2_2)
# TO AS 
#                          coef     se(coef) se2    Chisq  DF p      
#TreatAF                   -3.97100 0.2024   0.7272 384.88 1  1.1e-85
#TreatSF                   -2.43927 0.1934   0.3728 159.07 1  1.8e-36
#TreatSS                    0.06987 0.0534   0.2199   1.71 1  1.9e-01
#frailty(factor(total_h2o2                            0.30 0  3.3e-01
#Likelihood ratio test= 147  on 3 df,   p=<2e-16

total_h2o2_input$Treat <- relevel(total_h2o2_input$Treat, "SF")
total_cox_h2o2_3 <- coxph(Surv(time = total_h2o2_input$Day, event = total_h2o2_input$Mortality) ~ Treat+frailty(factor(total_h2o2_input$trial)), cluster=Treat, data=total_h2o2_input)
summary(total_cox_h2o2_3)
# TO SF
#                             coef   se(coef) se2    Chisq   DF p      
#TreatAS                    2.439 0.193404 0.3728   159.1 1  1.8e-36
#TreatAF                   -1.532 0.009887 0.7818 24002.5 1  0.0e+00
#TreatSS                    2.509 0.148185 0.3722   286.7 1  2.6e-64
#frailty(factor(total_h2o2                            0.3 0  3.3e-01
#Likelihood ratio test= 147  on 3 df,   p=<2e-16

total_h2o2_input$Treat <- relevel(total_h2o2_input$Treat, "SS")
total_cox_h2o2_4 <- coxph(Surv(time = total_h2o2_input$Day, event = total_h2o2_input$Mortality) ~ Treat+frailty(factor(total_h2o2_input$trial)), cluster=Treat, data=total_h2o2_input)
summary(total_cox_h2o2_4)
# TO SS
#                         coef     se(coef) se2    Chisq  DF p       
#TreatSF                   -2.50914 0.1482   0.3722 286.71 1   2.6e-64
#TreatAS                   -0.06987 0.0534   0.2199   1.71 1   1.9e-01
#TreatAF                   -4.04086 0.1565   0.7266 666.98 1  4.5e-147
#frailty(factor(total_h2o2                            0.30 0   3.3e-01
#Likelihood ratio test= 147  on 3 df,   p=<2e-16
...
#bonferroni p-adj value is 4*p-value
#AF to AS is 4.4e-85
#SF to SS is 1.04e-63
```
...
#no control
total_pa14_survfit_input<- total_pa14_input[1:192,1:3]
...
total_pa14
#saved as total_pa14_survplot_050923.pdf as landscape with 5x3 dim in Figures

#now make a plot that includes the control line
total_pa14_survfit_input_ctrl<- total_pa14_input[,1:3]
tail(total_pa14_survfit_input_ctrl)
total_pa14_survfit_input_ctrl<- total_pa14_survfit_input_ctrl %>%
  mutate(Treat=factor(Treat, levels=c("AF", "AS", "SF", "SS", "CA", "CS")))
total_sfit_pa14_ctrl <- survfit(Surv(Day, Mortality)~Treat, data=total_pa14_survfit_input_ctrl)
summary(total_sfit_pa14_ctrl)$table
total_pa14_ctrl <- ggsurvplot(total_sfit_pa14_ctrl, data=total_pa14_survfit_input_ctrl, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoFed","ApoStarved","SymFed","SymStarved","ControlApo","ControlSym"),
           palette=c("gray50", "purple", "gray50", "purple", "black", "black"), linetype = c("twodash", "twodash", "solid", "solid", "twodash", "solid"), xlab="Days Post Pa14 Infection", ggtheme=theme(panel.background = element_rect(color="black", fill="white"),
        axis.text = element_text(colour="black", size=10), 
        axis.title.x = ggtext::element_markdown(face="bold", size=12),
        axis.title.y = ggtext::element_markdown(face="bold",size=12),
        panel.grid.major = element_line(colour = "gray80", linewidth = 0.5, linetype="dotted"),
        ...
        legend.key.size=unit(10,"point"))
total_pa14_ctrl
#saved as total_pa14_survplot_ctrl_112623.pdf as landscape with 5x3 dim in Figures
...
#model with controls
total_pa14_input$Treat=factor(total_pa14_input$Treat,levels=c("AF","AS","SF","SS","CA","CS"))

total_cox_pa14_1 <- coxph(Surv(time = total_pa14_input$Day, event = total_pa14_input$Mortality) ~ Treat+frailty(factor(total_pa14_input$trial)), cluster=Treat, data=total_pa14_input)
summary(total_cox_pa14_1)
#TO AF
#                           coef     se(coef) se2       Chisq  DF p       
#TreatAS                     2.0211 0.38610     0.2510  27.40 1   1.7e-07
#TreatSF                     0.9102 0.29132     0.2300   9.76 1   1.8e-03
#TreatSS                    -0.3753 0.01325     0.2118 802.67 1  1.4e-176
#TreatCA                   -21.8398 1.01154  9321.8812 466.16 1  2.2e-103
#TreatCS                   -21.8398 1.01154  9321.8812 466.16 1  2.2e-103
#frailty(factor(total_pa14                               0.17 0   4.1e-01
#Likelihood ratio test= 204.1  on 2 df,   p=<2e-16

#total_cox_pa14_1_coxme <- coxme(Surv(time = total_pa14_input$Day, event = total_pa14_input$Mortality) ~ total_pa14_input$Treat+(1|total_pa14_input$trial), data=total_pa14_input)
#anova(total_cox_pa14_1,total_cox_pa14_1_coxme)

#use emmeans to control for multiple pairwise comparisons (would only work if we were accounting for the random effect in a different way and not through the frailty)
#emms<-emmeans(total_cox_pa14_1, ~Treat, adjust="Sidak")
#pairs(emms, interaction = "pairwise") %>% rbind(adjust="Sidak")


total_pa14_input$Treat <- relevel(total_pa14_input$Treat, "AS")
total_cox_pa14_2 <- coxph(Surv(time = total_pa14_input$Day, event = total_pa14_input$Mortality) ~ Treat+frailty(factor(total_pa14_input$trial)), cluster=Treat, data=total_pa14_input)
summary(total_cox_pa14_2)
# TO AS 
#                           coef    se(coef) se2       Chisq  DF p      
#TreatAF                    -2.021 0.38610     0.2510  27.40 1  1.7e-07
#TreatSF                    -1.111 0.09596     0.2194 134.01 1  5.4e-31
#TreatSS                    -2.396 0.38549     0.2518  38.64 1  5.1e-10
#TreatCA                   -23.861 1.13539  9321.8812 441.66 1  4.7e-98
#TreatCS                   -23.861 1.13539  9321.8812 441.66 1  4.7e-98
#frailty(factor(total_pa14                              0.17 0  4.1e-01
#Likelihood ratio test= 204.1  on 2 df,   p=<2e-16


total_pa14_input$Treat <- relevel(total_pa14_input$Treat, "SF")
total_cox_pa14_3 <- coxph(Surv(time = total_pa14_input$Day, event = total_pa14_input$Mortality) ~ Treat+frailty(factor(total_pa14_input$trial)), cluster=Treat, data=total_pa14_input)
summary(total_cox_pa14_3)
# TO SF
#                          coef     se(coef) se2       Chisq  DF p      
#TreatAS                     1.1109 0.09596     0.2194 134.01 1  5.4e-31
#TreatAF                    -0.9102 0.29132     0.2300   9.76 1  1.8e-03
#TreatSS                    -1.2855 0.29020     0.2256  19.62 1  9.4e-06
#TreatCA                   -22.7500 1.09389  9321.8812 432.53 1  4.6e-96
#TreatCS                   -22.7500 1.09389  9321.8812 432.53 1  4.6e-96
#frailty(factor(total_pa14                               0.17 0  4.1e-01
#Likelihood ratio test= 204.1  on 2 df,   p=<2e-16

total_pa14_input$Treat <- relevel(total_pa14_input$Treat, "SS")
total_cox_pa14_4 <- coxph(Surv(time = total_pa14_input$Day, event = total_pa14_input$Mortality) ~ Treat+frailty(factor(total_pa14_input$trial)), cluster=Treat, data=total_pa14_input)
summary(total_cox_pa14_4)
#TO SS
#                         coef     se(coef) se2       Chisq  DF p       
#TreatSF                     1.2855 0.29020     0.2256  19.62 1   9.4e-06
#TreatAS                     2.3964 0.38549     0.2518  38.64 1   5.1e-10
#TreatAF                     0.3753 0.01325     0.2118 802.67 1  1.4e-176
#TreatCA                   -21.4646 1.01198  9321.8812 449.88 1  7.6e-100
#TreatCS                   -21.4646 1.01198  9321.8812 449.88 1  7.6e-100
#frailty(factor(total_pa14                               0.17 0   4.1e-01
#Likelihood ratio test= 204.1  on 2 df,   p=<2e-16
...
#AF to AS is 6.8e-07
#AF to SF is 0.0072
#AF to SS is 5.6e-176
#AS to SF is 2.16e-30
#AS to SS is 2.04e-09
#SF to SS is 3.76e-05
```
...
#no controls
total_smarc_survfit_input<- total_smarc_input[1:192,1:3]
...
total_smarc
#saved as total_smarc_survplot_050923.pdf as landscape with 5x3 dim in Figures

#now make a plot that includes the control line
total_smarc_survfit_input_ctrl<- total_smarc_input[,1:3]
tail(total_smarc_survfit_input_ctrl)
total_smarc_survfit_input_ctrl<- total_smarc_survfit_input_ctrl %>%
  mutate(Treat=factor(Treat, levels=c("AF", "AS", "SF", "SS", "CA", "CS")))
total_sfit_smarc_ctrl <- survfit(Surv(Day, Mortality)~Treat, data=total_smarc_survfit_input_ctrl)
summary(total_sfit_smarc_ctrl)$table
total_smarc_ctrl <- ggsurvplot(total_sfit_smarc_ctrl, data=total_smarc_survfit_input_ctrl, alpha=0.6, size=1, censor.size=0, legend.labs=c("ApoFed","ApoStarved","SymFed","SymStarved","ControlApo","ControlSym"),
           palette=c("gray50", "purple", "gray50", "purple", "black", "black"), linetype = c("twodash", "twodash", "solid", "solid", "twodash", "solid"), xlab="Days Post Smarc Infection", ggtheme=theme(panel.background = element_rect(color="black", fill="white"),
        axis.text = element_text(colour="black", size=10), 
        axis.title.x = ggtext::element_markdown(face="bold", size=12),
        axis.title.y = ggtext::element_markdown(face="bold",size=12),
        panel.grid.major = element_line(colour = "gray80", linewidth = 0.5, linetype="dotted"),
        ...
        legend.key.size=unit(10,"point"))
total_smarc_ctrl
#saved as total_smarc_survplot_ctrl_112623.pdf as landscape with 5x3 dim in Figures
...
#add in control
total_smarc_input$Treat=factor(total_smarc_input$Treat,levels=c("AF","AS","SF","SS","CA","CS"))

total_cox_smarc_1 <- coxph(Surv(time = total_smarc_input$Day, event = total_smarc_input$Mortality) ~ Treat+frailty(factor(total_smarc_input$trial)), cluster=Treat, data=total_smarc_input)
summary(total_cox_smarc_1)
#TO AF
#                           coef     se(coef) se2       Chisq  DF p      
#TreatAS                     0.1145 0.04344     0.2082   6.95 1  8.4e-03
#TreatSF                     0.8026 0.12530     0.2178  41.03 1  1.5e-10
#TreatSS                    -1.1595 0.13626     0.2265  72.41 1  1.7e-17
#TreatCA                   -21.1851 1.02408  5828.8324 427.95 1  4.5e-95
#TreatCS                   -21.1851 1.02408  5828.8324 427.95 1  4.5e-95
#frailty(factor(total_smar                               0.19 0  4.0e-01
#Likelihood ratio test= 162.2  on 5 df,   p=<2e-16

total_smarc_input$Treat <- relevel(total_smarc_input$Treat, "AS")
total_cox_smarc_2 <- coxph(Surv(time = total_smarc_input$Day, event = total_smarc_input$Mortality) ~ Treat+frailty(factor(total_smarc_input$trial)), cluster=Treat, data=total_smarc_input)
summary(total_cox_smarc_2)
# TO AS 
#                           coef     se(coef) se2       Chisq  DF p      
#TreatAF                    -0.1145 0.04344     0.2082   6.95 1  8.4e-03
#TreatSF                     0.6881 0.13631     0.2192  25.48 1  4.5e-07
#TreatSS                    -1.2740 0.10673     0.2248 142.50 1  7.5e-33
#TreatCA                   -21.2996 1.01748  5828.8324 438.22 1  2.6e-97
#TreatCS                   -21.2996 1.01748  5828.8324 438.22 1  2.6e-97
#frailty(factor(total_smar                               0.19 0  4.0e-01
#Likelihood ratio test= 162.2  on 5 df,   p=<2e-16

total_smarc_input$Treat <- relevel(total_smarc_input$Treat, "SF")
total_cox_smarc_3 <- coxph(Surv(time = total_smarc_input$Day, event = total_smarc_input$Mortality) ~ Treat+frailty(factor(total_smarc_input$trial)), cluster=Treat, data=total_smarc_input)
summary(total_cox_smarc_3)
# TO SF
#                          coef     se(coef) se2       Chisq  DF p      
#TreatAS                    -0.6881 0.1363      0.2192  25.48 1  4.5e-07
#TreatAF                    -0.8026 0.1253      0.2178  41.03 1  1.5e-10
#TreatSS                    -1.9622 0.2419      0.2509  65.81 1  5.0e-16
#TreatCA                   -21.9878 1.0494   5828.8324 438.99 1  1.8e-97
#TreatCS                   -21.9878 1.0494   5828.8324 438.99 1  1.8e-97
#frailty(factor(total_smar                               0.19 0  4.0e-01
#Likelihood ratio test= 162.2  on 4 df,   p=<2e-16
               
total_smarc_input$Treat <- relevel(total_smarc_input$Treat, "SS")
total_cox_smarc_4 <- coxph(Surv(time = total_smarc_input$Day, event = total_smarc_input$Mortality) ~ Treat+frailty(factor(total_smarc_input$trial)), cluster=Treat, data=total_smarc_input)
summary(total_cox_smarc_4)
#TO SS
#                          coef    se(coef) se2       Chisq  DF p      
#TreatSF                     1.962 0.2419      0.2509  65.81 1  5.0e-16
#TreatAS                     1.274 0.1067      0.2248 142.50 1  7.5e-33
#TreatAF                     1.160 0.1363      0.2265  72.41 1  1.7e-17
#TreatCA                   -20.026 1.0036   5828.8324 398.13 1  1.4e-88
#TreatCS                   -20.026 1.0036   5828.8324 398.13 1  1.4e-88
#frailty(factor(total_smar                              0.19 0  4.0e-01
#Likelihood ratio test= 162.2  on 3 df,   p=<2e-16
